...
class sFlowExtendedGateway():
    ...
    def __init__(self, length, dataGram):
        ...
        dataPosition += 4
        self.dstAsPath = unpack('>{"i" * self.asPathCount}', dataGram[dataPosition:(dataPosition + self.asPathCount * 4)])
        dataPosition += self.asPathCount * 4
        self.communitiesCount = unpack('>i', dataGram[dataPosition:(dataPosition + 4)])[0]
        dataPosition += 4
        self.communities = unpack('>{"i" * communitiesCount}', dataGram[dataPosition:(dataPosition + communitiesCount * 4)])
...
class sFlowExtendedMpls():
    ...
    def __init__(self, length, dataGram):
        ...
        dataPosition += 4
        self.inLabelStack = unpack('>{"i" * self.inLabelStackCount}', dataGram[dataPosition:(dataPosition + self.inLabelStackCount * 4)])
        dataPosition += self.inLabelStackCount * 4
        self.outLabelStackCount = unpack('>i', dataGram[dataPosition:(dataPosition + 4)])[0]
        dataPosition += 4
        self.outLabelStack = unpack('>{"i" * self.outLabelStackCount}', dataGram[dataPosition:(dataPosition + self.outLabelStackCount * 4)])
...
class sFlowExtendedVlantunnel():
    ...
    def __init__(self, length, dataGram):
        ...
        self.stack = unpack('>{"i" * stackCount}', dataGram[4:(4 + stackCount * 4)])
